      ...
      <form onSubmit={handleSubmit} className="space-y-3 max-h-[80vh] overflow-y-auto pr-1">
        <div className="grid grid-cols-2 gap-2">
          <div className="col-span-2">
            <label className="block text-xs font-medium text-gray-700 mb-0.5">Seller (Sell To) *</label>
            <select
              value={clientId}
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-0.5">Received Amount</label>
  ...
  w.document.write(`<!DOCTYPE html><html><head><title>Invoice ${txn.invoiceNumber || txn.id.slice(0,8)}</title>
    <style>
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      .invoice-header { background: linear-gradient(135deg, #1e3a5f 0%, #2563eb 50%, #3b82f6 100%); color: white; padding: 28px 32px; }
      ...
      .total-section { margin: 20px 32px; padding: 20px 24px; background: #eff6ff; border-radius: 10px; border: 1px solid #bfdbfe; }
      .total-row { display: flex; justify-content: space-between; align-items: center; padding: 4px 0; font-size: 14px; color: #374151; }
      .total-row.grand { font-size: 20px; font-weight: 700; color: #1e3a5f; padding-top: 12px; margin-top: 8px; border-top: 2px solid #93c5fd; }
      ...
      .print-btn { display: block; margin: 0 auto 20px; padding: 10px 32px; background: #2563eb; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 14px; font-weight: 500; transition: background 0.2s; }
    ...
    <div class="invoice-wrapper">
      ...
      <div class="total-section">
        ...
        <div class="total-row"><span>Received Amount</span><span>₹${Number(txn.paidAmount).toLocaleString("en-IN")}</span></div>
...
function EditSaleModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Sale Entry">
      <form onSubmit={handleSubmit} className="space-y-3 max-h-[80vh] overflow-y-auto pr-1">
        ...
        <div className="px-5 pb-4 border-t border-gray-100">
          {txn.vehicleNumber && (
            <div className="flex gap-4 mt-2 text-xs text-gray-500">
              <span>Vehicle: {txn.vehicleNumber}</span>
            </div>
          )}
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {transactions.map((txn) => (
            <TransactionRow key={txn.id} txn={txn} onEdit={() => setEditTxn(txn)} onDelete={loadData} />
          ))}
        </div>
      )}
