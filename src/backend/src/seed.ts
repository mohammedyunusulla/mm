 ...
 *   SUPER_EMAIL     (default: super@mandi.app)
...
async function main() {
  const email = process.env.SUPER_EMAIL ?? "super@mandi.app";
  const password = process.env.SUPER_PASSWORD ?? "changeme123";

  const existing = await masterDb.superUser.findUnique({ where: { email } });
  if (existing) {
  ...
  await masterDb.superUser.create({ data: { email, passwordHash } });
